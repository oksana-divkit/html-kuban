...
$(function() {
	...
	$('.promo-slider').each(function() {
		...
		var $promoSliderWrap = $promoSlider.parent();

		var widthSlider = $promoSlider.width();
		$('.promo-slider__li').width(widthSlider);
		...
		var options = {
			...
			prevPage: $promoSliderWrap.find('.promo-slider__btn-prev'),
			nextPage: $promoSliderWrap.find('.promo-slider__btn-next'),
		...
		$(window).resize(function() {
			var widthSlider = $promoSlider.width(),
				$slides = $promoSlider.find('.promo-slider__li'),
				countSlides = $slides.size();

			$slides.width(widthSlider);

			slider.reload();
			$photoSliderWrap.find('.promo-slider__ul').width(widthSlider*countSlides);
		});
	});
	...
	$('.photo-slider').each(function() {
		var $photoSlider = $(this),
			$photoSliderWrap = $photoSlider.parent(),
			$photoSliderPeview = $photoSliderWrap.find('.photo-slider-preview');
		...
		var options = {
			...
			speed: 300,
			elasticBounds: 1,
			pagesBar: $photoSliderPeview,
			pageBuilder: function (idx) {
				return '<li class="photo-slider-preview__li">' + $photoSliderPeview.find('.photo-slider-preview__li').eq(idx).html() + '</li>';
			},
			activatePageOn: 'click',
			dragHandle: 1,
			dynamicHandle: 1,
		...
		var slider = new Sly($photoSlider, options).init();

		$(window).resize(function() {
			var widthSlider = $photoSlider.width(),
				$slides = $photoSlider.find('.photo-slider__slide'),
				countSlides = $slides.size();

			$slides.width(widthSlider);

			slider.reload();
			$photoSliderWrap.find('.photo-slider__ul').width(widthSlider*countSlides);
		});
	...
	$('.faq-list__title').click(function() {
		...
		$this.siblings('.faq-list__desc').slideToggle('slow', function() {
			$this.closest('.faq-list__item').toggleClass('open', $(this).is(':visible'));
		});
	});
